<<<<<<< HEAD
package com.lqr.mytopk.readHugeFile;
			...
			while (fcin.read(rBuffer)!=-1 && sufList.size()!=DEFAULT_SUF_LINE_NO) {//fcin.read(rBuffer)：从文件管道读取内容到缓冲区(rBuffer)
				...
				for(int i = 0; i < rSize; i++){
					if(bs[i] == LF){
						...
						//碰见关键词后开始统计 start
						if(hitTheKeyWorld && sufList.size()<DEFAULT_SUF_LINE_NO) {
							sufList.add(line);
						}
						//碰见关键词后开始统计 end
...
//				System.out.println(line);
			...
			//将获取到的前置数据， 后连数据， 输出
			//writeFileByLine(fcout, wBuffer, line + enter);
			for (int j = 0; j < DEFAULT_PRE_LINE_NO; j++) {
				writeFileByLine(fcout, wBuffer, preQueue.poll() + enter);
			}
			for (int j = 0; j < sufList.size(); j++) {
				writeFileByLine(fcout, wBuffer, sufList.get(j) + enter);
			}
...
public class RAndWHugeFileNIOCut {
	...
	private static String FILE_PATH = "D:\\20180723_001.txt";
	private static String TARGET_FILE_PATH_1 = "E:\\testlogs\\copy_method1.txt";
	private static String TARGET_FILE_PATH_2 = "D:\\20180723_001_cut.txt";
	
	private static int DEFAULT_PRE_LINE_NO = 3;
	private static int DEFAULT_SUF_LINE_NO = 4;//排除关键词那一行
	
	private static String DEFAULT_KEY_WORLD = "20180723";
	public static void main(String args[]) throws Exception {
...
>>>>>>> 0eedab9b550bdf36ed27108088fe6eec93717129
